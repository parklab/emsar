...
treenode* new_lasttreenode(char c,int val)
{
   treenode *newnode;
   newnode = (treenode*)malloc(sizeof(treenode));
   newnode->letter=c;
...
int search_treehash(char* s,treenode* node){
    int j,val;

    if(node==NULL) return(-1);
    if(strlen(s)==0) return(-1); /* length 0 string cannot be stored in this structure */
    if(strlen(s)==1) {
       ...
       for(j=0;j<node->size;j++) {
          if(node->next[j]->letter==s[0]) return( search_treehash(s+1,node->next[j]) ); 
       }
    }
    return(-1); /* the key string doesn't exist. */
}
